        ...
        <div class="col-md-12 grid-margin">
            @if (Auth::user()->hasRole('Admin'))
                <div class="d-flex justify-content-end">
                    <a href="{{route('periodos-escolares.create')}}" role="button"
                       class="btn btn-md btn-epca mr-2 d-flex align-items-center">
                        <i class="mdi mdi-plus green-epca"></i>Agregar Periodo Escolar
                    </a>
            ...
            <div class="card">
                <div class="card-body">
                    <h2 class="card-titleSection mb-3">
                        Periodos Escolares <i class="fas fa-calendar-alt fa-sm"></i>
                    </h2>
                                    ...
                                    <table id="data_table" class="table dataTable no-footer mt-5" role="grid"
                                        ...
                                        @foreach($school_periods as $school_period)
                                            <tr role="row" class="odd">
                                                <td>{{$school_period->year}}</td>
                                                <td>{{$school_period->description}}</td>
                                                <td width="10%">
                                                    <a href="" class='btn btn-option'
                                                       style='background-color:transparent;'>
                                                        <i class="fas fa-eye mr-0"></i>
                                                    </a>
                                                    <a href="{{route('periodos-escolares.edit',$school_period)}}"
                                                       class='btn btn-option'
                                                       style='background-color:transparent;'>
                                                        <i class="fas fa-pencil-alt mr-0"></i>
                                                    </a>
                                                    <button onclick="deleteData({{$school_period->id}})"
                                                            class='btn btn-option'
                                                            style='background-color:transparent;'
                                                            data-toggle="modal"
